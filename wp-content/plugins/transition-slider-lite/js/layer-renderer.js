...
STX.LayerRenderer = function(params) {
    ...
    this.loadFont = function(index) {
        ...
        if (fontFamily && fontFamily != 'initial') {
            ...
            WebFont.load({
                google: {
                    families: [fontFamily, fontFamily + ":" + element.fontWeight]
                },
                fontactive: function() {
    ...
    this.addNodeElement = function(index) {
        ...
        switch (el.type) {
            case "text":
                el.node = document.createElement(el.tag || "div");
                break;
        ...
        el.node.onclick = function(e) {
            self.onLayerClick(Number(this.dataset.id), e.shiftKey);
        };
