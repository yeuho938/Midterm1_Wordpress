...
if ( ! function_exists( 'newsever_post_thumbnail' ) ) :
    /**
     ...
     */
    function newsever_post_thumbnail() {
        ...
        if ( is_singular() ) :
            ...
            $theme_class = newsever_get_option('global_image_alignment');
            $post_image_alignment = get_post_meta($post->ID, 'newsever-meta-image-options', true);
            ...
            if ( $post_class != 'no-image' ):
                ...
                <div class="post-thumbnail <?php echo esc_attr($post_class); ?>">
                    <?php the_post_thumbnail('newsever-featured'); ?>
                </div>
            <?php endif; ?>

        <?php else :
            $archive_layout = newsever_get_option('archive_layout');
            $archive_layout = $archive_layout;
            $archive_class = '';
            if ($archive_layout == 'archive-layout-list') {
                $archive_image_alignment = newsever_get_option('archive_image_alignment');
                $archive_class = $archive_image_alignment;
                $archive_image = 'medium';
            } elseif ($archive_layout == 'archive-layout-full') {
                $archive_image = 'newsever-medium';
            } else {
