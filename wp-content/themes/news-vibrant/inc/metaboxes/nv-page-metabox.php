...
add_action( 'add_meta_boxes', 'news_vibrant_page_meta_options' );
 
if ( ! function_exists( 'news_vibrant_page_meta_options' ) ) :
    function  news_vibrant_page_meta_options() {
        add_meta_box(
            'news_vibrant_page_meta',
            esc_html__( 'Page Meta Options', 'news-vibrant' ),
            'news_vibrant_page_meta_callback',
            'page',
            'normal',
            'high'
        );
 ...
 */
if ( ! function_exists( 'news_vibrant_page_meta_callback' ) ) :
	function news_vibrant_page_meta_callback() {
		...
		wp_nonce_field( basename( __FILE__ ), 'news_vibrant_page_meta_nonce' );
?>
 ...
 * @since 1.0.8
 */
add_action( 'save_post', 'news_vibrant_save_page_meta' );

if ( ! function_exists( 'news_vibrant_save_page_meta' ) ) :

function news_vibrant_save_page_meta( $post_id ) {

    global $post, $nv_allowed_textarea;

    // Verify the nonce before proceeding.
    $news_vibrant_post_nonce   = isset( $_POST['news_vibrant_page_meta_nonce'] ) ? $_POST['news_vibrant_page_meta_nonce'] : '';
