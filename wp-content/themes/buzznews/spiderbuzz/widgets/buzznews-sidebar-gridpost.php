 ...
 * */
class Buzznews_Sidebar_Gridpost_Layout extends WP_Widget {

	/* Register Widget with WordPress*/
	function __construct() {
		parent::__construct(
			'buzznews_sidebar_gridview_section', // Base ID
			esc_html__( 'BN Sidebar: Grid Layout', 'buzznews' ), //Widget Name
			array( 'description' => esc_html__( 'Display sidebar grid post layout.', 'buzznews' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$home_blog_category_id = ( ! empty( $instance['category'] ) ) ? absint( $instance['category'] ) : esc_html__( 'all', 'buzznews' );
		$number_posts = ( ! empty( $instance['number_posts'] ) ) ? absint( $instance['number_posts'] ) : 6; 
		
		// last post count
        if( !empty( $home_blog_category_id ) ){
                ...
                <div class="buzznews-newsfeed-inner-wrapper">
					<div class="buzznews-trendingnews-right">
						...
						<div class="buzznews-postlist-arrow">
							<span class="buzznews-postlist-ajax arrow-left" catId="<?php echo esc_attr($home_blog_category_id); ?>" postCount="<?php echo esc_attr($number_posts); ?>" postPaginate="1"  postLastPaginate="<?php echo esc_attr($last_page_count); ?>" postDisplayStyle="SidebarGridLayout"  ><a href="javascript:void(0)"><i class="fa fa-angle-left" aria-hidden="true"></i></a></span>
							<span class="buzznews-postlist-ajax arrow-right"  catId="<?php echo esc_attr($home_blog_category_id); ?>" postCount="<?php echo esc_attr($number_posts); ?>" postPaginate="2"  postLastPaginate="<?php echo esc_attr($last_page_count); ?>" postDisplayStyle="SidebarGridLayout"  ><a href="javascript:void(0)"><i class="fa fa-angle-right" aria-hidden="true"></i></a></span>
						</div>
...
function buzznews_sidebar_gridpost_layout_config() {
    register_widget( 'Buzznews_Sidebar_Gridpost_Layout' );
}
add_action( 'widgets_init', 'buzznews_sidebar_gridpost_layout_config' );