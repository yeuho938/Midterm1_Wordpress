 ...
 * */
class Buzznews_Sidebar_List_Layout extends WP_Widget {

	/* Register Widget with WordPress*/
	function __construct() {
		parent::__construct(
			'buzznews_sidebar_post_list', // Base ID
			esc_html__( 'BN Sidebar: Post List', 'buzznews' ), //Widget Name
			array( 'description' => esc_html__( 'Display Sidebar Post List.', 'buzznews' ), ) // Args
		);
	}
                    ...
                    <div class="buzznews-trendingnews-inner-wrapper">
                        <div class="buzznews-trendingnews-right">
                            ...
                            <div class="buzznews-postlist-arrow">
                                <span class="buzznews-postlist-ajax arrow-left" catId="<?php echo esc_attr($home_blog_category_id); ?>" postCount="<?php echo esc_attr($number_posts); ?>" postPaginate="1"  postLastPaginate="<?php echo esc_attr($last_page_count); ?>"  postDisplayStyle="postList" ><a href="javascript:void(0)"><i class="fa fa-angle-left" aria-hidden="true"></i></a></span>
                                <span class="buzznews-postlist-ajax arrow-right"  catId="<?php echo esc_attr($home_blog_category_id); ?>" postCount="<?php echo esc_attr($number_posts); ?>" postPaginate="2"  postLastPaginate="<?php echo esc_attr($last_page_count); ?>" postDisplayStyle="postList" ><a href="javascript:void(0)"><i class="fa fa-angle-right" aria-hidden="true"></i></a></span>
                            </div>
...
function buzznews_sidebar_list_layout_config() {
    register_widget( 'Buzznews_Sidebar_List_Layout' );
}
add_action( 'widgets_init', 'buzznews_sidebar_list_layout_config' );