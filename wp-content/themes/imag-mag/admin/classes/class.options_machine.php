...
class imagmag_themepacific_Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'upload':
					if(!isset($value['mod'])) $value['mod'] = '';
					$output .= imagmag_themepacific_Options_Machine::optionsframework_uploader_function($value['id'],$value['std'],$value['mod']);			
				break;
				...
				case 'media':
					...
					if(!isset($value['mod'])) $value['mod'] = '';
					$output .= imagmag_themepacific_Options_Machine::optionsframework_media_uploader_function( $value['id'], $value['std'], $int, $value['mod'] ); // New AJAX Uploader using Media Library			
				break;
		...
		'Josefin Sans' => 'Josefin Sans Regular',
 		'Josefin Slab' => 'Josefin Slab Regular',
 		'Judson' => 'Judson',
		'Julee' => 'Julee',
		'Junge' => 'Junge',
		'Jura' => ' Jura Regular',	 
		...
		'Zeyada' => 'Zeyada',
				...
				case 'slider':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= imagmag_themepacific_Options_Machine::optionsframework_slider_function($value['id'],$value['std'],$oldorder,$order,$int);
					} else {
						$i = 0;
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ','imagmag').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
	...
	public static function optionsframework_uploader_function($id,$std,$mod){
		...
		$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">'.__('Upload','imagmag').'</span>';
