<?php
/**
 * Recent News Thumbnail Col Post Widget.
 *
 * @since 1.0.0
 */
if (!class_exists('Recent_News_Thumb_Col_Posts')) :
     ...
     * @since 1.0.0
     */
    class Recent_News_Thumb_Col_Posts extends WP_Widget
    {
        private function defaults()
        {
            $defaults = array(
                'title' => esc_html__('Thumbnail Three Col Posts', 'recent-news'),
                'cat' => 0,
                'post-number' => 6,
        ...
        {
            $opts = array(
                'classname' => 'recent-news-thumbnail-col-post',
                'description' => esc_html__('Help to display content in thumbnail image Layout three column.', 'recent-news'),
            );

            parent::__construct('refined-magazine-thumbnail-col-post', esc_html__('Refined Magazine Three Col Thumbnail Post', 'recent-news'), $opts);
        }
        ...
        public function form($instance)
        {
            ...
            <p>
                <label
                        for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'recent-news'); ?></label>
            ...
            <p class="custom-dropdown-posts">
                <label for="<?php echo esc_attr($this->get_field_name('cat')); ?>">
                    <strong><?php esc_html_e('Select Category: ', 'recent-news'); ?></strong>
                </label>
                <?php
                $cat_args = array(
                    'orderby' => 'name',
                    ...
                    'show_option_all' => esc_html__('Show Recent Posts', 'recent-news')
                );
            ...
            <p>
                <label
                        for="<?php echo esc_attr($this->get_field_id('post-number')); ?>"><?php esc_html_e('Number of Posts to Display:', 'recent-news'); ?></label>
...
if ( ! function_exists( 'recent_news_load_widgets' ) ) :
     ...
     * @since 1.0.0
     */
    function recent_news_load_widgets() {

        // Thumbnail Widget.
        register_widget( 'Recent_News_Thumb_Col_Posts' );

    }

endif;
add_action( 'widgets_init', 'recent_news_load_widgets' );